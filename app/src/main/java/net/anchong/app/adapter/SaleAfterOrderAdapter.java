 ...
 */
public class SaleAfterOrderAdapter extends BaseAdapter {

    private Context mContext;
    private LayoutInflater mInflater;
    private SaleAfterOrderFragment mFragmetn;
    ...
    public SaleAfterOrderAdapter(Context mContext, LayoutInflater mInflater, SaleAfterOrderFragment mFragmetn, OrderInfoResponse orderInfoResponse, List<OrderInfoResponse.ResultDataEntity.ListEntity> orders) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //先获取到每个订单
        OrderInfoResponse.ResultDataEntity.ListEntity orderItem = orders.get(position);
        //设置显示的商铺名称
        ...
        switch (state) {
            case "1":
                ...
                vh.action2.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(mContext, "付款功能还未开通！敬请期待！", Toast.LENGTH_SHORT).show();
                    }
                });
                break;
            case "2":
                vh.orderState.setText("等待已付款");
                vh.action.setVisibility(View.GONE);
                break;
            case "3":
                vh.orderState.setText("卖家已发货");
                vh.action.setVisibility(View.VISIBLE);
                vh.action1.setText("查看物流");
                vh.action1.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(mContext, "点击查看物流", Toast.LENGTH_SHORT).show();
                    }
                });
                vh.action2.setText("确认收货");
                vh.action2.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(mContext, "点击确认收货!", Toast.LENGTH_SHORT).show();
                    }
                });
                break;
            case "4":
