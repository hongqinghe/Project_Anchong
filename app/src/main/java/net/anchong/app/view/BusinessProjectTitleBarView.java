 ...
 */
public class BusinessProjectTitleBarView extends LinearLayout implements View.OnClickListener {
    ...
    private ImageView mGeneralTitleBarTitle;

    @ViewInject(R.id.titlebar_general_right)
    private TextView mGeneralTitleBarRight;

    public BusinessProjectTitleBarView(Context context) {
        super(context);
        mContext = context;
        initView();
    }

    public BusinessProjectTitleBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView();
    }
    ...
    private void initView() {
        View.inflate(mContext, R.layout.view_business_project_titlebar, this);
        x.view().inject(this);
    ...
    public void setData(String title, String action) {
        if ("1".equals(title)) {
            mGeneralTitleBarTitle.setImageResource(R.drawable.hot_project_text);
        }
        if ("2".equals(title)) {
            mGeneralTitleBarTitle.setImageResource(R.drawable.new_project_text);
        }
        if ("3".equals(title)) {
            mGeneralTitleBarTitle.setImageResource(R.drawable.hot_project);
        }
        if ("4".equals(title)) {
            mGeneralTitleBarTitle.setImageResource(R.drawable.deta);
        }
