...
import net.anchong.app.ui.MyPublishActivity;
...
public class UIDisplayer {
    ...
    /* 必须在UI线程中初始化handler */
    public UIDisplayer(final ProgressDialog progressDialog, Activity activity) {
            ...
            public void handleMessage(Message inputMessage) {
                switch (inputMessage.what) {
                    case UPLOAD_OK:
                        ...
                        Toast.makeText(UIDisplayer.this.activity, "图片上传成功", Toast.LENGTH_SHORT).show();
                        if (UIDisplayer.this.activity instanceof MyPublishActivity) {
                            MyPublishActivity mpa = (MyPublishActivity) UIDisplayer.this.activity;
                            mpa.updataUI();
                        }
                        if (UIDisplayer.this.activity instanceof CertificationActivity) {
                            CertificationActivity ca = (CertificationActivity) UIDisplayer.this.activity;
                            ca.updataUI();
                        }
                        if(UIDisplayer.this.activity instanceof RequestShopActivity){
                            RequestShopActivity shop = (RequestShopActivity)UIDisplayer.this.activity;
                            shop.updateUI();
                        }
                        if(UIDisplayer.this.activity instanceof CommnuityActivity){
                            CommnuityActivity community = (CommnuityActivity)UIDisplayer.this.activity;
                            community.updateUI();
                        }
                        break;
                    case UPLOAD_FAIL:
                        progressDialog.dismiss();
                        Toast.makeText(UIDisplayer.this.activity, "上传失败，请检查网络连接", Toast.LENGTH_SHORT).show();
                        break;
                    case DOWNLOAD_OK:
                        Toast.makeText(UIDisplayer.this.activity, "下载成功", Toast.LENGTH_SHORT).show();
                        break;
                    case SETTING_OK:
                        new AlertDialog.Builder(UIDisplayer.this.activity).setTitle("设置成功").setMessage("设置域名信息成功,现在<选择图片>, 然后上传图片").show();
                        break;
                    case DOWNLOAD_FAIL:
                        Toast.makeText(UIDisplayer.this.activity, "下载失败", Toast.LENGTH_SHORT).show();
                        break;
