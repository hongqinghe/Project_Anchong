...
import net.anchong.app.adapter.MyCommunityInfoAdapter;
...
import net.anchong.app.view.MyCommunityInfoHeaderView;
 ...
 */
public class MyCommunityInfoActivity extends BaseActivity implements XListView.IXListViewListener, CommunityInfoTitleBarView.CommunityTitleBarOnclickListener, MyCommunityInfoHeaderView.DelCommunityListener {
    ...
    @ViewInject(R.id.title_my_community_info)
    ...
    private MyCommunityInfoHeaderView myCommunityInfoHeaderView;
    private MyCommunityInfoAdapter adapter;


    public static void start(Context context, String chart_id) {
        Intent intent = new Intent(context, MyCommunityInfoActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_community_info);
        x.view().inject(this);
        // 沉浸式状态栏
...
//        initCommunitycom();
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (MainActivity.isLogin) {
                    ...
                    LoginActivity.start(MyCommunityInfoActivity.this);
                }
            }
        });
    }
    ...
    private void initView() {
        ...
        if (myCommunityInfoHeaderView == null) {
            myCommunityInfoHeaderView = new MyCommunityInfoHeaderView(this);
            myCommunityInfoHeaderView.setData(communityInfoResponse);
            myCommunityInfoHeaderView.setDelCommunityListener(this);
            xListView.addHeaderView(myCommunityInfoHeaderView);
        }
        ...
        if (communityComResponse != null && communityComResponse.getResultData().getList() != null && communityComResponse.getResultData().getList().size() > 0) {
            adapter = new MyCommunityInfoAdapter(this, getLayoutInflater(), this, communityComResponse.getResultData().getList());
            if (communityComResponse.getResultData().getList() != null && communityComResponse.getResultData().getTotal() <= 10) {
                xListView.setPullLoadEnable(false);
            }

        } else {
            adapter = new MyCommunityInfoAdapter(this, getLayoutInflater(), this, null);
            xListView.setPullLoadEnable(false);
        }
     ...
     * 收藏  取消收藏聊聊请求
     */
    private void testCollect(final String api) {
        //判断用户是否登录
        if (MainActivity.isLogin) {
            CommunityInfoRequest communityInfoRequest = new CommunityInfoRequest(chart_id);
            RequestModel requestModel = new RequestModel(System.currentTimeMillis() / 1000 + "", MyApplication.APP_VERSION + "", MainActivity.loginResponseModel.getResultData().getGuid(), communityInfoRequest);
            String requestJson = new Gson().toJson(communityInfoRequest);
            ...
            HttpManager.getInstance()
                    ...
                    .execute(ResponseErrorModel.class, new CommonCallback<ResponseErrorModel>() {
                        ...
                        public void onSuccess(ResponseErrorModel response, Object... obj) {
                            if ("0".equals(response.getServerNo())) {
                                if (api.equals(MyApplication.COMMUNITYCOMMUNITYDEL)) {
                                    finish();
                                }
    ...
    @Override
    public void clickDelCommunity() {
        testCollect(MyApplication.COMMUNITYCOMMUNITYDEL);
    }
}
