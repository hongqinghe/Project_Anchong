 ...
 */
public class BusinessHotSearchActivity extends BaseActivity implements BusinessProjectTitleBarView.GeneralTitleBarOnclickListener, XListView.IXListViewListener {
    ...
    public static void start(Context context, String type,String ad_name) {
        Intent intent = new Intent(context, BusinessHotSearchActivity.class);
        intent.putExtra("type", type);
        intent.putExtra("ad_name", ad_name);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ad_name = getIntent().getStringExtra("ad_name");
...
//                Toast.makeText(BusinessHotProjectActivity.this, hotprojectList.get(position-1).toString(), Toast.LENGTH_SHORT).show();
                ...
                BusinessDetaActivity.start(BusinessHotSearchActivity.this, businessModel, showPhone);
            }
        });
    }
    ...
    private void initData() {
        ...
        //数据检查完成，可以提交数据
        page = 1;
//        RecentProjectRequest recentProjectRequest = new RecentProjectRequest(page + "",ad_name);
        BusinessInfoParamModel businessInfoParamModel = new BusinessInfoParamModel(",",ad_name,"",page+"");
        String json = new Gson().toJson(businessInfoParamModel);
        ...
        if (MainActivity.isLogin == true) {
            requestModel = new RequestModel(System.currentTimeMillis() / 1000 + "", MyApplication.APP_VERSION + "",
                    MainActivity.loginResponseModel.getResultData().getGuid() + "", businessInfoParamModel);
